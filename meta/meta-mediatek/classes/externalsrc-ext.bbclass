# Extended version of externalsrc,
# aimed at supporting automatic switch to prebuilt while source folder does not exist.
...
python () {
    ...
    externalsrcprebuilt = externalsrc.replace("build/../src/", "build/../prebuilt/")

    externalsrcbuild = d.getVar('EXTERNALSRC_BUILD', True)
    externalsrcbuildprebuilt = externalsrcbuild.replace("build/../src/", "build/../prebuilt/")

    # Switch to prebuilt if necessary
    if not os.path.exists(externalsrc):
        if os.path.exists(externalsrcprebuilt):
            externalsrc = externalsrcprebuilt
            externalsrcbuild = externalsrcbuildprebuilt
        else:
            bb.warn("Both %s and %s aren't existed" % (externalsrc, externalsrcprebuilt) )
