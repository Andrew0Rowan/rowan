...
void mt_log_setup(unsigned int start, unsigned int size, unsigned int aee_buf_size)
{
    ...
    if(is_power_on_boot()) {
        ...
        if(p_atf_log_ctrl->info.atf_crash_flag == ATF_CRASH_MAGIC_NO) {
            ...
            dump_ret = mt_crash_log_dump((uint8_t*)(uintptr_t)p_atf_log_ctrl->info.atf_crash_log_addr, ATF_CRASH_LAST_LOG_SIZE);
    ...
    for(i=0; i<PLATFORM_CORE_COUNT; i++) {
        ...
        debug_print("mt_log_setup - atf_except_write_pos_per_cpu[%d]: 0x%x\n", i, p_atf_log_ctrl->info.atf_except_write_pos_per_cpu[i]);
    }
