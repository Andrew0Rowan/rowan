...
int32_t rpmb_init(void)
{
	...
	/* mmap 2MB aligned to save mmap table(xlat_table) size (level 1)
	   0x44A02000 ==> 0x44A00000
	   in case 4K-aligned is used, 4K page table size is needed.
	*/
	mmap_add_region((uint64_t)(phy_addr & ~(PAGE_SIZE_2MB_MASK)),
	                (uint64_t)(phy_addr & ~(PAGE_SIZE_2MB_MASK)),
	                PAGE_SIZE_2MB,
                           ...
                           uint32_t msg_len, u8 *mac)
{
	...
	*/
...
#ifdef MTK_RPMB_DEBUG
	printf("compute first hash ==> hash(i_key_pad ¡ü message)\n");
#endif //MTK_RPMB_DEBUG
	...
	printf("compute second hash ==> hash(o_key_pad ¡ühash1)\n");
#endif //MTK_RPMB_DEBUG
