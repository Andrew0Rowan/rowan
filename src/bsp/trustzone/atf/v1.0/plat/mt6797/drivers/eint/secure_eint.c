...
unsigned int deint_possible_irq[] = {206,207,208,209};
unsigned int MAX_DEINT_CNT = 4;
unsigned int EINT_MAX_CHANNEL = 192;
...
int mt_eint_clr_deint(unsigned int irq_num)
{
	...
	unsigned int ret;
        ...
        if (i == MAX_DEINT_CNT) {
                ERROR("%s: no matched possible deint irq for %u\n", __func__, irq_num);
                return -1;
        }
...
unsigned int mt_eint_set_sens(unsigned int eint_num, unsigned int sens)
{
        ...
        if (sens == MT_EDGE_SENSITIVE) {
                ...
                ERROR("%s invalid sensitivity value\n", __func__);
                return 0;
        }
        mt_reg_sync_writel(bit, base);
        printf("[EINT] %s :%lx, bit: %x\n", __func__, base, bit);
...
mt_eint_get_deint_sec_en (unsigned int deint_mapped)
{
        ...
        printf ("[EINT] %s :%x,value: 0x%x,bit: %x\n", __func__, base,st ,bit);
        return ((st & bit)?1:0);
}
...
int mt_eint_set_secure_deint(unsigned int eint_num, unsigned int irq_num)
{
	...
	int ret;
        if (eint_num >= EINT_MAX_CHANNEL) {
                ERROR("%s: eint_num(%u) is not in (0, %d)\n", __func__,
