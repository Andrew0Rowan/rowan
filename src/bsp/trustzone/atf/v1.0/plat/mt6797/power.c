...
//    flush_dcache_range(0x10222000,0x1000);
    ...
    mmio_write_32(SPM_MP2_CPUSYS_PWR_CON, tmp);//Assert pwr_rst_b
...
int big_sw_on_seq(int select){
    ...
    if(select == 0x10){
        //the SW workaround of PLL should be copy from HW mode, not all chips will meet the issue
...
void big_sw_off_seq(int select){
    ...
    if(select == 0x10){
        ...
        tmp = mmio_read_32(0x1022220c);//dummy read
        mmio_write_32(0x1022220c,tmp);//dummy write, avoid optimization
