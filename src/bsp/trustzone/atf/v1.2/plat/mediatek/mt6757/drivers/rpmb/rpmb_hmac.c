...
int32_t rpmb_init(void)
{
	...
	INFO("[%s] in rpmb shared mem smc, aligned phy_addr:0x%x offset:0x%16lx!!\n",
	       __func__, (phy_addr & ~(PAGE_SIZE_MASK)), offset);
	INFO("[%s] dump content 0x%16lx set in lk nwd:\n", __func__, (uint64_t)shared_mem_ptr);
	unsigned int i = 0;

	for (i = 0; i < 2; i++, shared_mem_ptr++) {
		INFO("0x%16lx:0x%16lx\n", (uint64_t)(shared_mem_ptr), *(shared_mem_ptr));
	}
...
int32_t rpmb_hmac(uint32_t magic, uint32_t size)
{
	...
	*/
	...
	if ((random_pattern != magic) || (random_pattern == 0)) {
		INFO("[%s] magic# is incorrect!!\n", __func__);
		return SIP_SVC_E_INVALID_PARAMS;
	}


#ifdef MTK_RPMB_DEBUG
	INFO("input magic#:0x%x\n", magic);
	INFO("generated random pattern:0x%x\n", random_pattern);
#endif //MTK_RPMB_DEBUG
