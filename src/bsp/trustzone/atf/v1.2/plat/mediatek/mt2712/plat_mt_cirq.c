...
__attribute__((weak)) uint32_t mt_cirq_get_mask(uint32_t cirq_num)
{
	...
	if (cirq_num >= CIRQ_IRQ_NUM) {
		tf_printf("[CIRQ] %s: invalid cirq %d\n", __func__, cirq_num);
		return -1;
	}
...
static int mt_cirq_set_sens(uint32_t cirq_num, uint32_t sens)
{
	...
	if (sens == MT_EDGE_SENSITIVE) {
		...
		tf_printf("[CIRQ] set_sens invalid sen value %d\n", sens);
		return -1;
	}
...
static int mt_cirq_set_pol(uint32_t cirq_num, uint32_t pol)
{
	...
	if (pol == MT_CIRQ_POL_NEG) {
		...
		tf_printf("[CIRQ] set_pol invalid polarity value %d\n", pol);
		return -1;
	}
...
void mt_cirq_flush(void)
{
	...
	if (cirq_pattern_clone_flush_check_val == 1) {
		...
		tf_printf("[CIRQ] cirq_pattern %d, cirq_p %d,",
		cirq_pattern_list, mt_cirq_get_pending(cirq_pattern_list));
		tf_printf("cirq_s %d, cirq_con 0x%x\n",
	...
	if (cirq_clone_flush_check_val == 1) {
		if (first_cirq_found) {
			tf_printf("[CIRQ] The first flush : CIRQ%d to IRQ%d\n",
				  first_flushed_cirq, first_irq_flushedto);
			tf_printf("[CIRQ] The last flush : CIRQ%d to IRQ%d\n",
				  last_fluashed_cirq, last_irq_flushedto);
		} else {
			tf_printf
			    ("[CIRQ] There are no pending interrupt in CIRQ");
			tf_printf("so no flush operation happend\n");
		}
		tf_printf
...
void mt_cirq_dump_reg(void)
{
	...
	tf_printf("[CIRQ] IRQ:\tPOL\tSENS\tMASK\n");
	for (cirq_num = 0; cirq_num < CIRQ_IRQ_NUM; cirq_num++) {
#if defined(__CHECK_IRQ_TYPE)
		...
		if (mask == 0) {
			...
			if (__check_irq_type[irq_iter].num ==
				...
				tf_printf
					...
					__check_irq_type[irq_iter].num);
				tf_printf("Mapping to wrong GIC num %d\n",
					CIRQ_TO_IRQ_NUM(cirq_num));
				pass = 0;
			}
		}
...
#if defined __CHECK_IRQ_TYPE
	tf_printf("[CIRQ] CIRQ Clone To GIC Verfication %s !\n",
		  pass == 1 ? "Pass" : "Failed");
#else
	tf_printf("[CIRQ] Plz enable __CHECK_IRQ_TYE");
	tf_printf("and update plat_mt_cirq.h for enable CIRQ Clone checking\n");
#endif
}
