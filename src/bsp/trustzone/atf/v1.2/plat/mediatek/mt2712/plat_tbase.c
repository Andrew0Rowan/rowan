...
/* Defines */
#define PLAT_TBASE_INPUT_HWIDENTITY	(0x1U)
#define PLAT_TBASE_INPUT_HWKEY		(0x2U)
#define PLAT_TBASE_INPUT_RNG		(0x3U)
 ...
 *
 * Supported command:
 *   PLAT_TBASE_INPUT_HWIDENTITY, request to get the HW unique key (could be public, but must be cryptographically unique).
...
static void coreDump(uint32_t coreId)
{
	...
	else
	{
		...
		gicv2_set_ipriorityr(MT_GIC_BASE, FIQ_SMP_CALL_SGI, GIC_HIGHEST_SEC_PRIORITY);
