...
/* Disable for ep */
			...
			case MTK_SIP_KERNEL_EMIMPU_SET_AARCH64:
				...
				//console_uninit();
				break;
			case MTK_SIP_KERNEL_LPDMA_READ_AARCH32:
			case MTK_SIP_KERNEL_LPDMA_READ_AARCH64:
				rc = (uint64_t)sip_lpdma_read(x1);
				break;
			case MTK_SIP_KERNEL_LPDMA_WRITE_AARCH32:
			case MTK_SIP_KERNEL_LPDMA_WRITE_AARCH64:
				rc = sip_lpdma_write(x1, x2);
				break;
			case MTK_SIP_KERNEL_LPDMA_GET_AARCH32:
			case MTK_SIP_KERNEL_LPDMA_GET_AARCH64:
				rc = sip_lpdma_get_mode();
				break;
			case MTK_SIP_KERNEL_DRAM_DCS_CHB_AARCH32:
			case MTK_SIP_KERNEL_DRAM_DCS_CHB_AARCH64:
				//console_init(gteearg.atf_log_port, ELBRUS_UART_CLOCK, ELBRUS_BAUDRATE);
				//INFO("Ahsin sip_dram_smc_chb x1=%x \n",(unsigned int) x1);
				rc = (uint64_t)sip_dram_smc_chb(x1);
				//console_uninit();
				break;
