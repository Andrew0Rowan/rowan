...
int etc_init(void)
{
	if (etc_read(0x10212240) & 0x01) { /* check unipll was enabled*/
		...
		etc_write(ATC_VOUT, 0x00AF);
		udelay(10);
		etc_write(ATC_SW_DORMANT, 0x0000);
		etc_write(ATC_VALARM_VERR, 0x0050);
		etc_write(ATC_OVER_CTRL, 0xB600);
		etc_write(ATC_DROOP_CTRL, 0x02FE);
		etc_write(ATC_DROOP_VERR, 0x5DA4);
		etc_write(ATC_VMON_CTRL, 0x1FFF);
		etc_write(ATC_VMON_STATUS, 0x0038);
		etc_write(ATC_TEST_ACCESS, 0x00A5);
		etc_write(ATC_TDMUX_CTRL, 0x0301);
		etc_write(ATC_TDMUX, 0x1F1F);

		etc_write(ATC_DVC0_LO, 0x0000);
		etc_write(ATC_DVC0_HI, 0x822D);
		etc_write(ATC_DVC_WRITE, 0x0000);

		etc_write(ATC_DVC1_LO, 0x0400);
		etc_write(ATC_DVC1_HI, 0x942D);
		etc_write(ATC_DVC_WRITE, 0x0001);

		etc_write(ATC_DVC2_LO, 0x0840);
		etc_write(ATC_DVC2_HI, 0xA62D);
		etc_write(ATC_DVC_WRITE, 0x0002);

		etc_write(ATC_DVC3_LO, 0x0C84);
		etc_write(ATC_DVC3_HI, 0xB82D);
