...
#define DMB()	__asm__ volatile("dmb ishst" : : : "memory")

/* updated by other cores than current one, MUST be atomically updated */
static atomic_t flush_L1_count;
/* updated by other cores than current one, MUST be atomically updated */
static atomic_t flush_L2_count;
/* only updated by the current core, could be not atomic */
static volatile int all_L1_flush_done;
/* only updated by the current core, could be not atomic */
static volatile int  L2_flush_check[PLATFORM_CORE_COUNT];
/* only updated by the current core, could be not atomic */
static volatile int all_cache_flush_done;
...
static void flush_cache(void *info)
{
	...
	/*
	 * an implicit dsb is in the end of dcsw_op_level1,
	 * so that we can confirm flush_L1_count is updated after L1 flush
	 */
	atomic_add(1, &flush_L1_count);

	while (!ACCESS_ONCE(all_L1_flush_done));

	if (ACCESS_ONCE(L2_flush_check[cpu])) {
		dcsw_op_level2(DCCISW);

		/*
	 	 * an implicit dsb is in the end of dcsw_op_level2,
	 	 * so that we can confirm flush_L2_count is updated after L2 flush
	 	 */
		atomic_add(1, &flush_L2_count);
	}

	while (!ACCESS_ONCE(all_cache_flush_done));
}

static int nr_1bit(unsigned int map)
{
	int i;
	int ret = 0;
...
#define PLATFORM_CPUMASK	((1 << PLATFORM_CORE_COUNT)-1)
	unsigned int cpu = platform_get_core_pos(read_mpidr());
	unsigned int cpus_should_flush_cache;
	int i = 0;
	int L1_check_count;
	int L2_check_count;
	...
	flush_L1_count.counter = 0;
	flush_L2_count.counter = 0;
	all_L1_flush_done = 0;
	all_cache_flush_done = 0;
	...
	cpus_should_flush_cache = ((~cpus_dont_flush_cache) & PLATFORM_CPUMASK)
					& (~(1 << cpu));

	L1_check_count = nr_1bit(cpus_should_flush_cache);

	set_fiq_cache_debug_step(0x4);

	/* ask other cores to flush L1, no wait!! */
	fiq_smp_call_function(cpus_should_flush_cache, flush_cache, 0, 0);
	...
	while (ACCESS_ONCE(L1_check_count) != ACCESS_ONCE(flush_L1_count.counter));
	...
	L2_check_count = 0;
	for (i = 0; i < PLATFORM_CORE_COUNT; ++i) {
		if (!((1 << i) & cpus_should_flush_cache))
			continue;
	...
	/*
	 * there is 2 cases for other cores after all_L1_flush_done set as 1:
	 * 1. start to flush L2 if it is the chosen one above
	 * 2. wait until all things done
	 */
	all_L1_flush_done = 1;
	...
	set_fiq_cache_debug_step(0xb);

	/* wait all cores flush L2 finish their work */
	while (ACCESS_ONCE(L2_check_count) != ACCESS_ONCE(flush_L2_count.counter));

	set_fiq_cache_debug_step(0xc);

	/* make sure the above loop exit and then proceed */
	DMB();

	/* OK, we can be free now */
	all_cache_flush_done = 1;

	set_fiq_cache_debug_step(0xd);
