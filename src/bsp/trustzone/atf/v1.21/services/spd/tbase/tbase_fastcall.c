...
void print_fastcall_params( char *msg, uint32_t secure ) {
  ...
  DBG_PRINTF("tbase %s (%d) 0x%lx 0x%lx 0x%lx 0x%lx\n\r", msg, secure,
...
{
  DBG_PRINTF( "tbase_fastcall_handler TBASE_SMC_FASTCALL_STATUS %lx %lx\n\r", x1, x2 );
  switch(maskSWdRegister(x1)) {
          ...
          void *cookie)
{
	...
	ret = bl31_fiq_filter(id, flags, handle, cookie);
	if (ret == EFIQ_SEL1_FOUND){
