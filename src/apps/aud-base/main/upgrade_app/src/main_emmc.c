...
int backup_recovery(const char *src_path ,const char force_update)
{
	char sub_image_name[50];
	int ret=-1;
	printf_d("start %s \n", __FUNCTION__);
	snprintf(sub_image_name,50,"%s/tmp/%s",src_path,"recovery.ramdisk.img");
	ret = upgrade_emmc_partition("BOOTIMG", sub_image_name, TRUE);
	if (ret < 0){
		printf_e("backup recovery partition fail !\n");
		return -1;
	}
	else
		printf_i("upgrade backup recovery partition ok !\n"); 
...
int get_emmc_flags(struct upgrade_flag_buffer *flag){
	int fd = -1;
	int ret = -1;
    ...
    bool part_exist = search_emmc_part_from_table("MISC", &part);
    if (FALSE == part_exist){
	   printf_e("misc partiton not existed!\n");
        return -1;
	}
    unsigned long long start_address = part.start_lba * G_BLOCK_SIZE;
	printf_d("misc start_address is at 0x%08lx \n",start_address);
	ssize_t flag_size = sizeof(*flag);
    char data[flag_size];
...
int set_emmc_flags(struct upgrade_flag_buffer *flag){
...
int main(int argc, char **argv)
{
    ...
    ret_mar = get_mbr_info();
	if(ret_mar){
     printf_e("read mbr failed!\n");
	}
	get_emmc_flags(&current_flag);
