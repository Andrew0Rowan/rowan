...
static VOID _upgrade_usb_state_process(BOOL b_mnt)
{
    if (b_mnt)
    {
...
INT32 _upg_control_call_app_to_flash_data(UPGRADE_IMAGE_INFO *imginfo, INT32 unflashed_size, BOOL is_last_flash_unit)
{
    INT32 i4_ret = 0;
    UPGRADE_IMAGE_INFO *img_info = NULL;
    CHAR *flash_img_name = NULL;
    INT32 flash_offset = 0;
    BOOL  is_all_img_flashed = FALSE;
...
INT32 _upg_control_sectionally_flash_img_data(DOWNLOAD_TREAD_PARAM *dl_param)
{
    ...
    BOOL is_img_download_done = FALSE;
    BOOL is_all_img_flashed = FALSE;
...
{
    INT32 i4_ret = 0;
    BOOL is_full_pkg = dl_param->is_full_pkg_update;
