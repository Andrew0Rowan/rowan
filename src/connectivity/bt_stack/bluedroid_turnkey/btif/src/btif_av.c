...
#if defined(MTK_A2DP_SNK_AAC_CODEC) && (MTK_A2DP_SNK_AAC_CODEC == TRUE)
        }else if (p_data->avk_config.codec_type == BTA_AV_CODEC_M24)
        {
...
static bt_status_t init_src_ext( btav_src_ext_callbacks_t* ext_callbacks )
{
    BTIF_TRACE_EVENT("## %s ##", __FUNCTION__);
    bt_status_t result = BT_STATUS_SUCCESS;

    if (bt_av_src_ext_callbacks)
    {
...
static bt_status_t btif_av_get_codec_config(bt_bdaddr_t *bd_addr)
{
    ...
    switch (codectype)
    {
        ...
        case BTIF_AV_CODEC_AAC:
            ...
            if (FALSE == bta_av_co_audio_get_aac_aptx_config((tA2D_AAC_CIE*)&codec_info, &minmtu, BTIF_AV_CODEC_AAC ))
            {
        ...
        case 0xFF:
            ...
            BTIF_TRACE_EVENT("codectype is aptX");
            if (FALSE == bta_av_co_audio_get_aac_aptx_config((tA2D_APTX_CIE*)&codec_info, &minmtu, NON_A2DP_MEDIA_CT))
            {
                BTIF_TRACE_ERROR("%s.get aptX codec info error", __func__);
            }
#endif
            break;
        default:
...
static void btif_av_src_adjust_bitpool(UINT8 bitpool)
{
    BTIF_TRACE_EVENT("## %s ##", __FUNCTION__);
