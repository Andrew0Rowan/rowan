	...
	for(i = 0; (i < times) || !finalResult; i += nextInterval) {
		...
		if ((rxOk + rxErr) == 0) {
			...
			fprintf(stdout, "[%3d] (%d)RX Total OK Count: %4d  /(%d)RX Tolal ERR Count: %4d / PER: %2d .. / Rx Total Count: %4d"
					" (%d)RSSI0: %i / RSSI1: %i \r\n", i, ret[0], RxTotalCount, ret[1], rxErr,
					(100 * rxErr)/(RxTotalCount + rxErr), (RxTotalCount + rxErr), ret[2], (signed char)(rxRssisFinal & BITS(0,7)), (signed char)(rxRssisFinal >> 8));
		}
		fflush(stdout);
