...
Configuration::Configuration():m_appConfigFile("/data/misc/dueros/app_config.json"),
                               m_duerosConfigFile("/data/misc/dueros/dueros_config.json"),
...
bool Configuration::readConfig() {
    ...
    if (access(m_duerosConfigFile.c_str(), F_OK) == 0) {
        ...
        if (readDocument.HasMember("wifi_ssid")) {
            m_wifiSsid = readDocument["wifi_ssid"].GetString();
        }

        if (readDocument.HasMember("wifi_pwd")) {
            m_wifiPwd = readDocument["wifi_pwd"].GetString();
        }

		APP_INFO("readconfig(m_wifiSsid = %s m_wifiPwd = %s)",
			m_wifiSsid.c_str(),m_wifiPwd.c_str());
...
void Configuration::setCommVol(unsigned int volume) {
    ...
    if (access(m_duerosConfigFile.c_str(), F_OK) == 0) {
        ...
        streamAlv << m_alertsVolume;
        addPairToDoc(document, "alert_volume", streamAlv.str());
        std::stringstream streamSsid;
        streamSsid << m_wifiSsid;
        addPairToDoc(document, "wifi_ssid", streamSsid.str());
        std::stringstream streamPwd;
        streamPwd << m_wifiPwd;
        addPairToDoc(document, "wifi_pwd", streamPwd.str());
...
std::string Configuration::getWifiSsid() const {
    return m_wifiSsid;
}


std::string Configuration::getWifiPwd() const {
    return m_wifiPwd;
}

void Configuration::setWifiSsidAndPwd(const std::string ssid,const std::string pwd) {
    m_wifiSsid = ssid;
	m_wifiPwd = pwd;
    if (access(m_duerosConfigFile.c_str(), F_OK) == 0) {
        rapidjson::Document document;
