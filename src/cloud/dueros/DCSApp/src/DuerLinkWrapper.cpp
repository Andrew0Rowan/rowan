...
void DuerLinkWrapper::initDuerLink() {
    ...
    DuerLinkMtkInstance::get_instance()->init_config_network_parameter(duerLink::platform_type::EOther,
                                                     duerLink::auto_config_network_type::EBle,
	...
	DuerLinkMtkInstance::get_instance()->init_duer_link_log();
    ...
    switch (notify_type) {
        ...
        case duerLink::ENetworkLinkFailed: {
            ...
            if (DuerLinkMtkInstance::get_instance()->get_operation_type() != operation_type::EAutoEnd) {
				...
				Configuration::getInstance()->setWifiSsidAndPwd("","");
...
void DuerLinkWrapper::ntpSerivceSetupNotify() {
    APP_INFO("ntpSerivceSetupNotify start");

    if (m_callback) {
        m_callback->detectNTPReady();
    }
}

bool DuerLinkWrapper::waitFor(
    const DuerLinkNetworkStatus networkStatus,
    const std::chrono::seconds duration) {
    std::unique_lock<std::mutex> lock(m_mutex);
    return m_trigger.wait_for(
        lock, duration, [this, networkStatus]() { return networkStatus == m_networkStatus; });
}
