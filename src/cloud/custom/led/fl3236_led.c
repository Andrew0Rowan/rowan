...
static int fl3236_led_voice_blinking_working = 0;
static int fl3236_led_connect_blinking_working = 0;
static pthread_t fl3236_bootup_tid;
static pthread_t fl3236_wakeup_tid;
static pthread_t fl3236_voice_blinking_tid;
static pthread_t fl3236_connect_blinking_tid;
...
{
    return ioctl(fl3236_led_fd,CMD_GROUP_ALL,GEN_GROUP_ALL(FL3236_LED_REG,FL3236_LED_OFF));
}
...
int fl3236_led_off(int led)
{
    ...
    return ioctl(fl3236_led_fd,CMD_GROUP_LINEAR,GEN_GROUP_LINEAR((FL3236_LED_REG + 3 * led),FL3236_LED_OFF, FL3236_LED_OFF, FL3236_LED_OFF));
}
...
void fl3236_led_thinking_show() {
    int i = 0, j = 0, loopcount = 0;
    printf("NEW LED  fl3236_led_thinking is called!!\n");
    fl3236_leds_off();
    for (i = 0; i < REG_GROUP_COUNT; i++) {
        if ((i % 2) == 0)
...
void fl3236_led_bootup_start() {
    int ret;
    printf("led_bootup_start is called!\n");
    fl3236_led_bootup_working = 1;

    ret = pthread_create(&fl3236_bootup_tid, NULL, (void *)fl3236_led_bootup_thread, NULL);
    if (ret < 0) {
...
void fl3236_led_voice_detect_thread(void) {
    int i;
    fl3236_leds_off();
    fl3236_leds_color(0, 0, 255);
...
void fl3236_led_voice_detect_start() {
    int ret;
    printf("led_wakeup_start is called!\n");
    fl3236_led_voice_detect_working = 1;
    ret = pthread_create(&fl3236_wakeup_tid, NULL, (void *)fl3236_led_voice_detect_thread, NULL);
    if (ret < 0) {
...
void fl3236_led_voice_detect_stop() {
...
void fl3236_led_voice_blinking_thread(void) {
    int i;
    fl3236_leds_off();
    fl3236_leds_color(0, 0, 255);
    for (i = 0; i < REG_GROUP_COUNT; i += 3)
    fl3236_led_color(i, 0, 253, 255);
    fl3236_leds_on(LED_LEVEL_DEF);
    while (fl3236_led_voice_blinking_working) {
        _led_sleep(30);
    }
    fl3236_leds_off();
}

void fl3236_led_voice_blinking_start() {
    int ret;
    printf("fl3236_led_voice_blinking_start is called!\n");
    fl3236_led_voice_detect_working = 1;
    ret = pthread_create(&fl3236_voice_blinking_tid, NULL, (void *)fl3236_led_voice_blinking_thread, NULL);
    if (ret < 0) {
        fl3236_led_voice_blinking_working = 0;
        printf("fl3236_led_voice_blinking thread creat failed!\n");
    }
    return;
}

void fl3236_led_voice_blinking_stop() {
    printf("led voice blinking stop!\n");
    if (fl3236_led_voice_blinking_working) {
        fl3236_led_voice_blinking_working = 0;
        pthread_join(fl3236_voice_blinking_tid, 0);
    }
}

void fl3236_led_connect_blinking_thread(void) {
    int i;
    fl3236_leds_off();
    fl3236_leds_color(0, 0, 255);
    for (i = 0; i < REG_GROUP_COUNT; i += 3)
    fl3236_led_color(i, 0, 253, 255);
    fl3236_leds_on(LED_LEVEL_DEF);
    while (fl3236_led_connect_blinking_working) {
        _led_sleep(30);
    }
    fl3236_leds_off();
}

void fl3236_led_connect_blinking_start() {
    int ret;
    printf("led_connect_blinking_start is called!\n");
    fl3236_led_connect_blinking_working = 1;
    ret = pthread_create(&fl3236_connect_blinking_tid, NULL, (void *)fl3236_led_connect_blinking_thread, NULL);
    if (ret < 0) {
        fl3236_led_connect_blinking_working = 0;
        printf("led connect blinking thread creat failed!\n");
    }
    return;
}

void fl3236_led_connect_blinking_stop() {
    printf("led connect blinking stop!\n");
    if (fl3236_led_connect_blinking_working) {
        fl3236_led_connect_blinking_working = 0;
        pthread_join(fl3236_connect_blinking_tid, 0);
    }
}

void fl3236_led_set_volume(int volume) {
