...
static void msdc_set_mclk(struct msdc_host *host, unsigned char timing, u32 hz)
{
	...
	dev_err(host->dev, "sclk: %d, timing: %d hz:%d cfg:0x%x\n", host->sclk, timing, hz,
...
unsigned int autok_debug_level;
...
static int msdc_drv_probe(struct platform_device *pdev)
{
	...
	host->src_clk = devm_clk_get(&pdev->dev, "sdio-src");
	if (IS_ERR(host->src_clk)) {
		ret = PTR_ERR(host->src_clk);
		goto host_free;
	}

	host->bus_clk = devm_clk_get(&pdev->dev, "sdio-bus");
	if (IS_ERR(host->bus_clk)) {
		ret = PTR_ERR(host->bus_clk);
		goto host_free;
	}

	host->cg_clk = devm_clk_get(&pdev->dev, "sdio-src-cg");
	if (IS_ERR(host->cg_clk)) {
		ret = PTR_ERR(host->cg_clk);
		goto host_free;
	}
