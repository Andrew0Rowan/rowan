...
//	ulong begin = get_timer(0);
	...
	RTC_Write_PMIC(MT6392_FQMTR_CON0, 0x0000); //disable freq. meter, gated src clock
	RTC_Write_PMIC(MT6392_TOP_CKPDN1, RTC_Read_PMIC(MT6392_TOP_CKPDN1) | RG_FQMTR_PDN); //TOP_CKPDN1[5]=1, gated fixed clock
	RTC_Write_PMIC(MT6392_TOP_RST_CON, RTC_Read_PMIC(MT6392_TOP_RST_CON) | RG_FQMTR_RST);			//FQMTR reset
	while( !(RTC_Read_PMIC(MT6392_FQMTR_CON2)==0) && (FQMTR_BUSY&RTC_Read_PMIC(MT6392_FQMTR_CON0))==FQMTR_BUSY);
	RTC_Write_PMIC(MT6392_TOP_RST_CON, RTC_Read_PMIC(MT6392_TOP_RST_CON) & ~RG_FQMTR_RST);			//FQMTR normal
	RTC_Write_PMIC(MT6392_TOP_CKPDN1, RTC_Read_PMIC(MT6392_TOP_CKPDN1) & ~RG_FQMTR_PDN); //TOP_CKPDN1[5]=0, turn on fixed clock

	RTC_Write_PMIC(MT6392_FQMTR_CON1, window_size); //set freq. meter window value (0=1X32K(fix clock))
	RTC_Write_PMIC(MT6392_FQMTR_CON0, FQMTR_EN | measureSrc); //enable freq. meter, set measure clock to 26Mhz
...
#endif	
	ret = RTC_Read_PMIC(MT6392_FQMTR_CON2);				//read data should be closed to 26M/32k = 812.5		
	return ret;
}
...
static void rtc_measure_four_clock(U16 *result)
{
	...
	RTC_Write_PMIC(MT6392_TOP_CKCON1, (RTC_Read_PMIC(MT6392_TOP_CKCON1) & FQMTR_FIX_CLK_26M) ); //select 26M as fixed clock
	window_size = 4;
	mdelay(1);
	result[0] = get_frequency_meter(0, FQMTR_AUD26M_CK, window_size); 		//select 26M as target clock
	
	//select XOSC_DET as fixed clock  
	RTC_Write_PMIC(MT6392_TOP_CKCON1, (RTC_Read(MT6392_TOP_CKCON1) | FQMTR_FIX_CLK_XOSC_32K_DET));
	window_size = 4;
	mdelay(1);
	result[1] = get_frequency_meter(0, FQMTR_AUD26M_CK, window_size); 		//select 26M as target clock
	
	//select 26M as fixed clock
	RTC_Write_PMIC(MT6392_TOP_CKCON1, (RTC_Read_PMIC(MT6392_TOP_CKCON1) & FQMTR_FIX_CLK_26M));
	...
	RTC_Write_PMIC(MT6392_TOP_CKCON1, (RTC_Read_PMIC(MT6392_TOP_CKCON1) | FQMTR_FIX_CLK_EOSC_32K));
	window_size = 4;
	mdelay(1);
...
static void rtc_switch_mode(bool XOSC, bool recovery)
{
	if (XOSC)
	{
		if (recovery)
		{
			...
			RTC_Write_PMIC(MT6392_CHRSTATUS, (RTC_Read_PMIC((MT6392_CHRSTATUS) | RTC_HW_DET_BYPASS) & RTC_HW_XOSC_MODE));
		}
	...
	} else 
	{
		if (recovery)
		{
			...
			RTC_Write_PMIC(MT6392_CHRSTATUS, (RTC_Read_PMIC(MT6392_CHRSTATUS) | RTC_HW_DET_BYPASS | RTC_HW_DCXO_MODE));
		}
...
static bool rtc_init_after_recovery(void)
{
	...
	RTC_Write_PMIC(MT6392_CHRSTATUS, (RTC_Read_PMIC(MT6392_CHRSTATUS) & ~0x0800));
...
static U16 eosc_cali(void)
{
	...
	RTC_Write_PMIC(MT6392_TOP_CKCON1, (RTC_Read_PMIC(MT6392_TOP_CKCON1) | FQMTR_FIX_CLK_EOSC_32K) );//fix clock = eosc 32K
...
void rtc_boot_check(void)
{
	...
	RTC_Write_PMIC(MT6392_TOP_CKPDN1, 0x3df);
	...
	RTC_Write_PMIC(MT6392_TOP_CKTST2, RTC_Read_PMIC(MT6392_TOP_CKTST2) | 0x0110);
	
	RTC_Write_PMIC(MT6392_TOP_CKPDN1, 0x3ff);
	rtc_clean_mark();
...
{
	int ret;
	u16 data[RTC_OFFSET_COUNT], data_alarm[RTC_OFFSET_COUNT];
	u32 con, con1, osc, chrstatus;

	data[RTC_OFFSET_SEC] = 0;
	data[RTC_OFFSET_MIN] = 0;
	data[RTC_OFFSET_HOUR] = 0;
	data[RTC_OFFSET_DOM] = 1;
	data[RTC_OFFSET_MTH] = 1;
	data[RTC_OFFSET_YEAR] = 2010 - 1968;

	data_alarm[RTC_OFFSET_SEC] = 0;
	data_alarm[RTC_OFFSET_MIN] = 0;
	data_alarm[RTC_OFFSET_HOUR] = 0;
	data_alarm[RTC_OFFSET_DOM] = 1;
	data_alarm[RTC_OFFSET_MTH] = 1;
	data_alarm[RTC_OFFSET_YEAR] = 1970 - 1968;


	ret = regmap_write(mt_rtc->regmap, MT6392_TOP_CKPDN1, 0x3df);
	if (ret < 0)
		goto exit;
	...
	ret = regmap_write(mt_rtc->regmap, mt_rtc->addr_base + RTC_PROT, RTC_PROT_UNLOCK2);
	if (ret < 0)
		goto exit;
	mtk_rtc_write_trigger(mt_rtc);
	...
	dev_err(mt_rtc->dev, "mtk_rtc_init con is 0x%x!\n", con);
	
	con |= RTC_CON_GPEN | RTC_CON_GOE;
//	con &= ~RTC_CON_F32KOB;
	con |= RTC_CON_F32KOB;

	dev_err(mt_rtc->dev, "mtk_rtc_init 1 con is 0x%x!\n", con);

	rtc_write(RTC_CON, con);
	Write_trigger();
	dev_err(mt_rtc->dev, "mtk_rtc_init 1 con is 0x%x!\n", rtc_read(RTC_CON));
#if 0
	ret = regmap_write(mt_rtc->regmap, mt_rtc->addr_base + RTC_CON, con);
	if (ret < 0)
		goto exit;
	mtk_rtc_write_trigger(mt_rtc);
#endif
	ret = regmap_read(mt_rtc->regmap, mt_rtc->addr_base + RTC_CON, &con1);
	if (ret < 0)
		goto exit;
	dev_err(mt_rtc->dev, "mtk_rtc_init con1 is 0x%x!\n", con1);

	ret = regmap_write(mt_rtc->regmap, mt_rtc->addr_base + RTC_POWERKEY1, RTC_POWERKEY1_KEY);
	if (ret < 0)
		goto exit;
	mtk_rtc_write_trigger(mt_rtc);

	ret = regmap_write(mt_rtc->regmap, mt_rtc->addr_base + RTC_POWERKEY2, RTC_POWERKEY2_KEY);
	if (ret < 0)
		goto exit;
	mtk_rtc_write_trigger(mt_rtc);
	mdelay(1000);
	...
	ret = regmap_read(mt_rtc->regmap, MT6392_CHRSTATUS, &chrstatus);
	if (ret < 0)
		goto exit;

	chrstatus |= RTC_HW_DET_BYPASS | RTC_HW_DCXO_MODE;

	ret = regmap_write(mt_rtc->regmap, MT6392_CHRSTATUS, chrstatus);
	if (ret < 0)
		goto exit;

	

	ret = regmap_write(mt_rtc->regmap, mt_rtc->addr_base + RTC_OSC32CON, RTC_OSC32CON_UNLOCK1);
	if (ret < 0)
		goto exit;
//	mtk_rtc_write_trigger(mt_rtc);
	mdelay(1);

	ret = regmap_read(mt_rtc->regmap, mt_rtc->addr_base + RTC_OSC32CON, &osc);
	if (ret < 0)
		goto exit;
	dev_err(mt_rtc->dev, "mtk_rtc_init 1 osc is 0x%x !\n", osc);

	ret = regmap_write(mt_rtc->regmap, mt_rtc->addr_base + RTC_OSC32CON, RTC_OSC32CON_UNLOCK2);
	if (ret < 0)
		goto exit;
//	mtk_rtc_write_trigger(mt_rtc);
	mdelay(1);
	...
	if (ret < 0)
		goto exit;
	dev_err(mt_rtc->dev, "mtk_rtc_init 2 osc is 0x%x !\n", osc);

	ret = regmap_write(mt_rtc->regmap, mt_rtc->addr_base + RTC_OSC32CON, 0x240F);
	if (ret < 0)
		goto exit;
//	mtk_rtc_write_trigger(mt_rtc);
	mdelay(1);
	...
	if (ret < 0)
		goto exit;
	dev_err(mt_rtc->dev, "mtk_rtc_init osc is 0x%x!\n", osc);
...
//	mtk_rtc_write_trigger(mt_rtc);
	...
	ret = regmap_write(mt_rtc->regmap, mt_rtc->addr_base + RTC_OSC32CON, 0x24D1);
	if (ret < 0)
		goto exit;
//	mtk_rtc_write_trigger(mt_rtc);
	mdelay(1);
	...
	ret = regmap_bulk_write(mt_rtc->regmap, mt_rtc->addr_base + RTC_TC_SEC,
					data, RTC_OFFSET_COUNT);
	mtk_rtc_write_trigger(mt_rtc);
	
	ret = regmap_bulk_write(mt_rtc->regmap, mt_rtc->addr_base + RTC_AL_SEC,
					data_alarm, RTC_OFFSET_COUNT);
	mtk_rtc_write_trigger(mt_rtc);
...
exit:
	dev_err(mt_rtc->dev, "mtk_rtc_init regmap write/read error!!!\n");
}
#endif
