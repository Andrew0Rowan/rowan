...
static void set_tc_trigger_hw_protect(int temperature, int temperature2)
{
	...
	if (temperature2 > -275000)
		THERMAL_WRAP_WR32(temp | 0x80000000, TEMPMONINT);	/* enable trigger middle & Hot SPM interrupt */
	else
...
static void thermal_cal_prepare(struct device *dev)
{
	...
	calefuse2 = buf[1];
	calefuse3 = buf[2];
	pr_err("[Power/CPU_Thermal] [Thermal calibration] buf[0]=0x%x, buf[1]=0x%x, buf[2]=0x%x\n", buf[0], buf[1], buf[2]);

	if (buf[0] & BIT(0)) {
		g_adc_ge_t      = ((buf[1] & 0xFFC00000)>>22);/*ADC_GE_T [9:0] *(0x10009184)[31:22]*/
		g_adc_oe_t      = ((buf[1] & 0x003FF000)>>12);/*ADC_OE_T [9:0] *(0x10009184)[21:12]*/
		g_o_vtsmcu1     = (buf[0] & 0x03FE0000)>>17;  /*O_VTSMCU1    (9b) *(0x10009180)[25:17]*/
		g_o_vtsmcu2     = (buf[0] & 0x0001FF00)>>8;   /*O_VTSMCU2    (9b) *(0x10009180)[16:8]*/
		g_o_vtsmcu3 		= (buf[2] & 0xFF800000) >> 23;	/* O_VTSMCU3    (9b) *(0x10009188)[31:23] */
		g_o_vtsmcu4 		= (buf[2] & 0x007FC000) >> 14;	/* O_VTSMCU4    (9b) *(0x10009188)[22:14] */
		g_o_vtsabb      = (buf[1] & 0x00000FF8) >> 3;      /*O_VTSABB     (9b) *(0x10009184)[11:3]*/
		g_degc_cali     = (buf[0] & 0x0000007E)>>1;   /*DEGC_cali    (6b) *(0x10009180)[6:1]*/
		g_adc_cali_en_t = (buf[0] & 0x00000001);      /*ADC_CALI_EN_T(1b) *(0x10009180)[0]*/
		g_o_slope       = (buf[0] & 0xFC000000)>>26;  /*O_SLOPE      (6b) *(0x10009180)[31:26]*/
		g_o_slope_sign  = (buf[0] & 0x00000080)>>7;   /*O_SLOPE_SIGN (1b) *(0x10009180)[7]*/
		g_id            = (buf[1] & 0x00000004)>>2;   /*ID           (1b) *(0x10009184)[2]*/
		if (g_id == 0)
			g_o_slope = 0;
	} else {
