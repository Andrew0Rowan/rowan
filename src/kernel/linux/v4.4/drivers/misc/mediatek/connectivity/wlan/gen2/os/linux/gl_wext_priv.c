...
int priv_driver_set_suspend_mode(IN struct net_device *prNetDev, IN char *pcCommand, IN int i4TotalLen)
{
	...
	if (i4Argc >= 2) {
		...
		if (fgIsUnderSuspend == fgEnable) {
			DBGLOG(REQ, INFO, "%s: Already in suspend mode, SKIP!\n", __func__);
			i4BytesWritten = snprintf(pcCommand, i4TotalLen,
				"Already in suspend mode, SKIP!");
			return i4BytesWritten;
		}

		if (fgEnable == TRUE) {
			DBGLOG(REQ, INFO, "wlanHandleSystemSuspend() Call\n");
			wlanHandleSystemSuspend();
			if (prGlueInfo->prAdapter->fgIsP2PRegistered)
				p2pHandleSystemSuspend();
		} else if (fgEnable == FALSE) {
			DBGLOG(REQ, INFO, "wlanHandleSystemResume() Call\n");
			wlanHandleSystemResume();
			if (prGlueInfo->prAdapter->fgIsP2PRegistered)
				p2pHandleSystemResume();
		}

		i4BytesWritten = snprintf(pcCommand, i4TotalLen, "Set suspend mode %d", fgEnable);
	}
