...
void wlanMonWorkHandler(struct work_struct *work)
{
	...
	if (prGlueInfo->fgIsEnableMon) {
		...
		prGlueInfo->prMonDevHandler =
		    alloc_netdev_mq(sizeof(P_GLUE_INFO_T), NIC_MONITOR_INF_NAME,
		...
		*((P_GLUE_INFO_T *) netdev_priv(prGlueInfo->prMonDevHandler)) = prGlueInfo;
		...
		kalMemZero(&prGlueInfo->prMonDevHandler->stats, sizeof(struct net_device_stats));
