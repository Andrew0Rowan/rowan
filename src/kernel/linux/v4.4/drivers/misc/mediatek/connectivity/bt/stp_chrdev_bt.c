...
#define BT_FWLOG 1
#if BT_FWLOG
#define BT_DRIVER_NAME_FWLOG "mtk_stp_bt_chrdev_fwlog"

static INT32 BT_devs_fwlog = 1;
static struct cdev BT_cdev_fwlog;
#if CREATE_NODE_DYNAMIC
static struct class *stpbt_class_fwlog;
static struct device *stpbt_dev_fwlog;

static bool open_flag_fwlog = false;
static bool write_flag_fwlog = false;
static bool read_flag_fwlog = false;
static UINT16 opcode_fwlog = 0;
#define FWLOG_I_LEN_FWLOG 16
static UINT8 i_buf_fwlog[FWLOG_I_LEN_FWLOG];
static UINT8 o_buf_fwlog[BT_BUFFER_SIZE];
#endif
#endif
...
#if BT_FWLOG
	/* check event */
	if (write_flag_fwlog == true
			&& i_buf[0] == 0x4
			&& opcode_fwlog == (i_buf[4] | (i_buf[5] << 8))) {
		write_flag_fwlog = false;
		read_flag_fwlog = true;
		memcpy(i_buf_fwlog, i_buf,
			(retval < FWLOG_I_LEN_FWLOG ? retval : FWLOG_I_LEN_FWLOG));
...
#if BT_FWLOG
static int BT_open_fwlog(struct inode *inode, struct file *file)
{
	BT_INFO_FUNC("major %d minor %d (pid %d), flag %d\n", imajor(inode), iminor(inode), current->pid, open_flag_fwlog);

	if (open_flag_fwlog != true)
		return -EIO;

	return 0;
}

static int BT_close_fwlog(struct inode *inode, struct file *file)
{
	BT_INFO_FUNC("major %d minor %d (pid %d), flag %d\n", imajor(inode), iminor(inode), current->pid, open_flag_fwlog);
...
ssize_t BT_write_fwlog(struct file *filp, const char __user *buf, size_t count, loff_t *f_pos)
{
	INT32 retval = 0;
	int i, j;
	char *p_buf = NULL;
	...
	if (count > BT_BUFFER_SIZE * 3) {
		BT_ERR_FUNC("%s: command is larger than maximum length, count = %zd\n", __func__, count);
		retval = -EIO;
		goto OUT;
	}
	...
	memset(i_buf_fwlog, 0, FWLOG_I_LEN_FWLOG);
	memset(o_buf_fwlog, 0, BT_BUFFER_SIZE);

	for (i = 0, j = 0; i < count; i++) {
		if (p_buf[i] == ' ' || p_buf[i] == '\t' || p_buf[i] == '\r' || p_buf[i] == '\n')
			continue;
		if ((p_buf[i] == '0' && p_buf[i + 1] == 'x')
			|| (p_buf[i] == '0' && p_buf[i + 1] == 'X')) {
			i++;
			continue;
		}
		...
		if ((p_buf[i] >= '0' && p_buf[i] <= '9')
			...
			if ((p_buf[i+1] >= '0' && p_buf[i+1] <= '9')
				|| (p_buf[i+1] >= 'A' && p_buf[i+1] <= 'F')
				|| (p_buf[i+1] >= 'a' && p_buf[i+1] <= 'f')) {
				i++;
				temp_str[1] = p_buf[i];
			}
			ret = (char) (kstrtol((char *)temp_str, 16, &res) & 0xff);
			o_buf_fwlog[j++] = res;
		}
	}

	if (j > 0) {
		retval = mtk_wcn_stp_send_data(o_buf_fwlog, j, BT_TASK_INDX);
		if (retval < 0)
			BT_ERR_FUNC("mtk_wcn_stp_send_data fail, retval %d\n", retval);
		else if (retval == 0) {
			/* Device cannot process data in time, STP queue is full and no space is available for write,
			 * native program should not call BT_write with no delay.
			 */
			BT_ERR_FUNC("Packet length %d, sent bytes %d, no space is available!\n", j, retval);
			retval = -EAGAIN;
		} else {
			BT_DBG_FUNC("Packet length %d, sent bytes %d\n", j, retval);
			if (j != retval) {
				retval = -EINVAL;
			} else {
				if (o_buf_fwlog[0] == 0x01) {
					write_flag_fwlog = true;
					read_flag_fwlog = false;
					opcode_fwlog = (o_buf_fwlog[1] | (o_buf_fwlog[2] << 8));
				}
			}
		}
...
ssize_t BT_read_fwlog(struct file *filp, char __user *buf, size_t count, loff_t *f_pos)
{
	INT32 retval = 0;
	char str[FWLOG_I_LEN_FWLOG * 3];
	char *ptr = str;
	UINT8 len, i;

	down(&rd_mtx);

	if (read_flag_fwlog == true) {
		len = (i_buf_fwlog[2] + 3) < FWLOG_I_LEN_FWLOG ?
			(i_buf_fwlog[2] + 3) : FWLOG_I_LEN_FWLOG;
		for (i = 0; i < len; i++)
			ptr += sprintf(ptr, " %02X", i_buf_fwlog[i]);

		BT_ERR_FUNC("FW read event:%s\n", str);
	} else
		BT_ERR_FUNC("read flag is %d\n", read_flag_fwlog);
...
const struct file_operations BT_fops_fwlog = {
	.open = BT_open_fwlog,
	.release = BT_close_fwlog,
	.read = BT_read_fwlog,
	.write = BT_write_fwlog,
};
#endif
...
static int BT_init(void)
{
	...
	INT32 cdev_err = 0;
#if BT_FWLOG
	dev_t dev_fwlog = MKDEV(BT_major, 1);
	INT32 alloc_ret_fwlog = 0;
	INT32 cdev_err_fwlog = 0;
#endif
...
#if BT_FWLOG
	/* Allocate char device */
	alloc_ret_fwlog = register_chrdev_region(dev_fwlog, BT_devs_fwlog, BT_DRIVER_NAME_FWLOG);
	if (alloc_ret_fwlog) {
		BT_ERR_FUNC("Failed to register device numbers\n");
		return alloc_ret_fwlog;
	}

	cdev_init(&BT_cdev_fwlog, &BT_fops_fwlog);
	BT_cdev_fwlog.owner = THIS_MODULE;

	cdev_err_fwlog = cdev_add(&BT_cdev_fwlog, dev_fwlog, BT_devs_fwlog);
	if (cdev_err_fwlog)
		goto error;

#if CREATE_NODE_DYNAMIC /* mknod replace */
	stpbt_class_fwlog = class_create(THIS_MODULE, "stpbtfwlog");
	if (IS_ERR(stpbt_class_fwlog))
		goto error;
	stpbt_dev_fwlog = device_create(stpbt_class_fwlog, NULL, dev_fwlog, NULL, "stpbtfwlog");
	if (IS_ERR(stpbt_dev_fwlog))
		goto error;
#endif
#endif
...
#if BT_FWLOG
#if CREATE_NODE_DYNAMIC
	if (stpbt_dev_fwlog && !IS_ERR(stpbt_dev_fwlog)) {
		device_destroy(stpbt_class_fwlog, dev_fwlog);
		stpbt_dev_fwlog = NULL;
	}
	if (stpbt_class_fwlog && !IS_ERR(stpbt_class_fwlog)) {
		class_destroy(stpbt_class_fwlog);
		stpbt_class_fwlog = NULL;
	}
#endif
	if (cdev_err_fwlog == 0)
		cdev_del(&BT_cdev_fwlog);

	if (alloc_ret_fwlog == 0)
		unregister_chrdev_region(dev_fwlog, BT_devs_fwlog);
#endif
